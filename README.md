...
10.11.2025:
Plan/goal for today: Fix the problems with the join_coLiving.html + create/update profile.html to fulfill the CRUD requirements. Without session() functionality (in PHP) most of the functionalities in the webapplication doesn't do anything. Therefore I need to figuere out the Java equivalent for PHP session(). Important: privacy/sequrity, stay in the session even when refreshing or changing site. After this is completed I need to make the logout.html file where the session is ended. Reminder/tips to myself: Ideally there should be two different (...). One for the users/residents and one for the ones who rent out. There should be a function in the "rent out/landlord" version that sends out the invite code on either phone or e-mail or prints it out. Proposal to how I can devide the two: When logging in it checks if the user is a resident or landlord, and gives accsess to diferent functionalities based on this. But then, how is the role of the user registered?
