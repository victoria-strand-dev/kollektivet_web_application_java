...
# Open webapplication here: https://tittentei.eu/
...
Plan/goal for today: Continue on yesterdays plan.
...
20.11.2025:
Today's goal: Use the session function I got to work last time, to fix the functionality for profile.html where one should be able to update one's user based on who's logged in. Profile.html is where the requierd CRUD functionalities will be fulfilled. The create part and read part will be fulfilled in in login.html, and update and delete will be fulfilled in profile. Potentially the delete part will be moved to settings.html which is more common in "real" webapplications. Now the session part  works with both login=session start and logout=session end. Update in profile works. It checks for who is logged in, and updates that user. I got no delete function yet. Next project is to add a delete function. Then I want to fix calender.html by making it look nice and by adding the functions that uses session + coLivingId to only showing events for the coliving the user is a part of. The events must be put in the calendar. There should be possible to skip between months, but that is not the most important thing for now.
